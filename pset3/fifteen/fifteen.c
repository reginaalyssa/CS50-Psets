...
void init(void)
{
    // determine first value to put in array
    int num = (d * d) - 1;
    ...
    for (int i = 0; i < d; i++)
    {
        for (int j = 0; j < d; j++)
        {
            // if odd, reverse positions of 1 and 2 on board
            if ((num == 2) && (isOdd == true))
            {
                board[i][j] = num - 1;
            }
            else if ((num == 1) && (isOdd == true))
            {
                board[i][j] = num + 1;
            }
            // fill 2D array with values from highest to lowest
            else
            {
                board[i][j] = num;
            }
